...
**Scoring system — deep dive:** Research and clearly document the full scoring system. Key questions: What is the scoring scale? How many waves count toward a surfer's heat total? What criteria do judges use to score individual waves (commitment, difficulty, innovation, variety, speed, power, flow — or others)? How does the judging panel work (how many judges, how scores are aggregated, whether outlier scores are dropped)? Have there been documented changes to judging criteria or emphasis over the years? What happens in unusual situations like interference calls — how does a penalty affect the non-offending surfer's score? How does a second interference work?
...
**Judging criteria differential weighting hypothesis:** Research whether the five judging criteria (commitment, difficulty, innovation, variety, speed/power/flow) contribute equally to actual scores awarded, or whether certain criteria (particularly aerial/progressive maneuvers) carry disproportionate weight. If a risk-reward tradeoff exists between high-scoring but low-completion-rate maneuvers vs. safer traditional surfing, document it — this could vary by surfer style and may differ between men's and women's competition. This hypothesis is tested during Phase 5 feature engineering.

**Men's vs. women's judging differences:** Research whether judging criteria, scoring scales, or judging tendencies differ between the men's and women's circuits. Investigate structural differences that may affect scoring patterns: aerial maneuver frequency and feasibility, historical wave condition allocation (do women's events receive equivalent competition windows?), and field size differences. Assess whether these differences are significant enough to warrant separate models for each circuit, and document the reasoning in the Decision Log.

**Interference analysis:** Research how interference penalties and priority violations work and how they affect heat outcomes. Document the penalty structure (first offense, second offense/disqualification) and assess whether some surfers are chronically aggressive with priority. Flag interference data as a collection requirement for Phases 3-4.
